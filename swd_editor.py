...
# This uses the environment variable 'HF_TOKEN'. 
# You can also hardcode it here for local testing: HF_TOKEN = "hf_..."
...
def format_saver(old_str, new_val):
    ...
    try: val = float(new_val)
    except: return str(new_val)
...
def process_updates(updates_list, current_lines):
    ...
    for update in updates_list:
        raw_name = str(update.get('parameter_name', '')).upper().replace(" ", "_")
...
def main():
    ...
    with open(INPUT_FILENAME, "r") as f: session_lines = f.readlines()
    ...
    while True:
        ...
        try:
            response = client.chat_completion(
                ...
                messages=[
                    {"role": "system", "content": "Return JSON ONLY. Format: {\"updates\": [{\"parameter_name\": \"...\", \"mode\": \"set/add/sub\", \"value\": 0.0}]}"},
            ...
            if data and 'updates' in data:
                ...
                with open(OUTPUT_FILENAME, "w") as f: f.writelines(session_lines)
            else:
